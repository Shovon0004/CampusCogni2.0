...
import { Search, User, MapPin, Calendar, Star, Briefcase, CheckCircle } from "lucide-react"
            ...
            {candidate.skills.slice(0, 4).map((skill, index) => {
              ...
              return (
                <Badge key={index} variant="outline" className="text-xs flex items-center gap-1">
                  {isVerified && <CheckCircle className="w-3 h-3 text-blue-500" />}
                  {skill}
                  {isVerified && verifiedSkill?.verificationScore && (
                    <span className="text-blue-600">({verifiedSkill.verificationScore}%)</span>
                  )}
                </Badge>
              )
            })}
